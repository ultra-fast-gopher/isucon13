...
func getUserStatisticsHandler(c echo.Context) error {
	...
	query = `SELECT u.id, IFNULL(SUM(l2.tip), 0) as total_tips FROM users u INNER JOIN livestreams l ON l.user_id = u.id LEFT JOIN livecomments l2 ON l2.livestream_id = l.id GROUP BY u.id;`
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	_, err = getLivestream(ctx, tx, livestreamID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
